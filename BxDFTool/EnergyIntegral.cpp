...
void SAverageFresnelIntegral::ExecuteConductor( uint32_t threadIndex, uint32_t localLaneIndex, uint32_t globalLaneIndex )
{
    float etaT = m_EtaBegin + localLaneIndex * m_EtaInterval;
    float etaI = m_EtaI;
    if ( m_Invert ) std::swap( etaI, etaT );
    float k = m_kInterval * threadIndex;
    float* samples = new float[ m_CosThetaCount ];
    float cosThetaInterval = 1.0f / ( m_CosThetaCount - 1 );
    for ( uint32_t i = 0; i < m_CosThetaCount; ++i )
    {
        float cosTheta = std::max( i * cosThetaInterval, 0.00001f );
        samples[ i ] = FresnelConductor( cosTheta, etaI, etaT, k ) * cosTheta;
    }
    float integral = 2.0f * NumericalIntegration::CompositeTrapezoidal( 0.0f, 1.0f, samples, m_CosThetaCount );
    delete[] samples;
    m_OutputBuffer[ globalLaneIndex ] = integral;
}

void SAverageFresnelIntegral::ExecuteDielectric( uint32_t threadIndex, uint32_t localLaneIndex, uint32_t globalLaneIndex )
{
    float etaT = m_EtaBegin + localLaneIndex * m_EtaInterval;
    float etaI = m_EtaI;
    if ( m_Invert ) std::swap( etaI, etaT );
    ...
    {
        float cosTheta = std::max( i * cosThetaInterval, 0.00001f );
        samples[ i ] = FresnelDielectric( cosTheta, etaI, etaT ) * cosTheta;
    }
    float integral = 2.0f * NumericalIntegration::CompositeTrapezoidal( 0.0f, 1.0f, samples, m_CosThetaCount );
    delete[] samples;
    m_OutputBuffer[ globalLaneIndex ] = integral;
